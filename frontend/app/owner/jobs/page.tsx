...
type Snapshot = { generated_at:string; counts:Record<string,number>; supported_job_types:string[]; jobs:Job[]; status?:string };
const empty:Snapshot={generated_at:new Date().toISOString(),counts:{},supported_job_types:[],jobs:[],status:'setup'};
...
export default function JobsPage(){
  ...
  async function runNext(){setLoading(true);setError('');setNotice('');try{const result=await request('/run-next',{method:'POST',body:JSON.stringify({limit:10})});setNotice(`Processed ${result.processed||0} job(s).`);await load();}catch(e){setError(e instanceof Error?e.message:'Unable to run jobs');}finally{setLoading(false);}}
  ...
  return <main className={styles.page}>
    <header className={styles.header}><div><span className={styles.eyebrow}>AUTOMATION RELIABILITY</span><h1>Job Operations Center</h1><p>Run tenant-scoped work with persistent status, bounded retries, exponential backoff, and dead-letter visibility.</p></div><div className={styles.actions}><button onClick={()=>void runNext()} disabled={loading}>Run next jobs</button><button onClick={()=>void enqueueEvents()} disabled={loading}>Queue event processing</button><button onClick={()=>void load()} disabled={loading}>Refresh</button><a className={styles.linkButton} href="/owner/system-health">System Health</a><a className={styles.linkButton} href="/owner">Control Plane</a></div></header>
    {notice&&<div className={styles.notice}>{notice}</div>}{error&&<div className={styles.error}>{error}</div>}
    <section className={styles.metrics}><article><span>Queued</span><strong>{c.queued||0}</strong></article><article><span>Running</span><strong>{c.running||0}</strong></article><article><span>Retry</span><strong>{c.retry||0}</strong></article><article><span>Completed</span><strong>{c.completed||0}</strong></article><article><span>Dead letter</span><strong>{c.dead_letter||0}</strong></article></section>
