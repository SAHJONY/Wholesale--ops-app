...
export default function IntegrationOperationsPage(){
  ...
  const load=useCallback(async(override?:string)=>{
    ...
    const results=await Promise.allSettled([
      request(HUB_API,'/snapshot',{},override),
      request(RELIABILITY_API,'/snapshot',{},override),
    ]);
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.metrics}><article><span>Provider API</span><strong>{hubOnline?'ONLINE':'SETUP'}</strong></article><article><span>Reliability API</span><strong>{reliabilityOnline?'ONLINE':'SETUP'}</strong></article><article><span>Providers</span><strong>{data.summary.providers}</strong></article><article><span>Configured</span><strong>{data.summary.configured}</strong></article><article><span>Open alerts</span><strong>{reliability.summary.open}</strong></article><article><span>Critical</span><strong>{reliability.summary.critical}</strong></article></section>
