...
export default function DealExecutionCenter() {
  ...
  const request = useCallback(async(path:string, options:RequestInit={}, override?:string) => {
    ...
    if (!active) throw new Error('Sign in on the owner dashboard first.');
  ...
  const load = useCallback(async(override?:string) => {
    ...
    } catch(err) { setError(err instanceof Error ? err.message : 'Unable to load deal execution'); }
    finally { setLoading(false); }
  },[request,dealId]);

  useEffect(()=>{ const stored=window.localStorage.getItem(SESSION_STORAGE)||''; setToken(stored); if(stored) void load(stored); },[load]);
  ...
  if(!token) return <main className={styles.setup}><section className={styles.setupCard}><h1>Owner session required</h1><p>Sign in on the owner dashboard first.</p><a className={styles.linkButton} href="/owner">Owner sign in</a></section></main>;
