...
export default function OwnerWorkspace() {
  ...
  const loadWorkspace = useCallback(async (tokenOverride?: string) => {
    ...
    try {
      const [me, pipelineData, leadData, followUpData, teamData, executiveData] = await Promise.all([
        ...
        request('/executive/command-center', {}, token),
      ]);
      ...
      setTeam(teamData);
      setCommandCenter(executiveData);
    } catch (err) {
  ...
  async function resetPassword(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${API_URL}/human-auth/reset-password`, {
        method: 'POST', cache: 'no-store', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email.trim().toLowerCase(), code: resetCode.trim(), password: newPassword }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(typeof data.detail === 'string' ? data.detail : 'Unable to reset password');
      setPassword(newPassword); setNewPassword(''); setResetCode('');
      setNotice('Password updated. Sign in with your new password.'); setAuthMode('login');
  ...
  async function runAutonomousCycle() {
    ...
    try {
      const result = await request('/executive/run-cycle', { method: 'POST', body: JSON.stringify({ execute_limit: 25 }) });
      setLastCycle({ executed: result.executed || 0, completed: result.completed || 0, failed: result.failed || 0 });
      setNotice(`Autonomous cycle finished: ${result.completed || 0} completed, ${result.failed || 0} failed.`);
      await loadWorkspace();
  ...
  async function decideApproval(id: number, decision: 'approved' | 'rejected') {
    ...
    try {
      await request(`/workspace/approvals/${id}/decision`, { method: 'POST', body: JSON.stringify({ decision }) });
  ...
  async function addTeamMember(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    try {
      await request('/auth/team', { method: 'POST', body: JSON.stringify({ name: form.get('name'), email: form.get('email'), role: form.get('role') }) });
      formElement.reset(); setNotice('Team member added.'); await loadWorkspace();
  ...
  function signOut() {
    ...
    window.localStorage.removeItem(SESSION_STORAGE);
    setSessionToken(''); setPrincipal(null); setPipeline(null); setCommandCenter(null); setLeads([]); setFollowUps([]); setTeam([]); setError(''); setNotice('');
  ...
  if (!principal) {
    return <main className={styles.setup}><section className={styles.setupCard}>
      ...
      {authMode === 'login' && <form onSubmit={signIn} className={styles.form}>
        <label htmlFor="owner-email"><b>Email</b></label><input id="owner-email" value={email} onChange={e => setEmail(e.target.value)} type="email" autoComplete="email" required />
        <label htmlFor="owner-password"><b>Password</b></label><input id="owner-password" value={password} onChange={e => setPassword(e.target.value)} type="password" autoComplete="current-password" required />
        <button disabled={loading}>{loading ? 'Signing in…' : 'Sign in'}</button><button type="button" className={styles.secondaryButton} onClick={() => setAuthMode('request-reset')}>Forgot password?</button>
      </form>}
      {authMode === 'request-reset' && <form onSubmit={requestReset} className={styles.form}>
        <label htmlFor="reset-email"><b>Email</b></label><input id="reset-email" value={email} onChange={e => setEmail(e.target.value)} type="email" required />
        <button disabled={loading}>{loading ? 'Sending…' : 'Send reset code'}</button><button type="button" className={styles.secondaryButton} onClick={() => setAuthMode('login')}>Back to sign in</button>
      </form>}
      {authMode === 'reset-password' && <form onSubmit={resetPassword} className={styles.form}>
        <label><b>Email</b></label><input value={email} onChange={e => setEmail(e.target.value)} type="email" required />
        <label><b>Six-digit code</b></label><input value={resetCode} onChange={e => setResetCode(e.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" required minLength={6} maxLength={6} />
        <label><b>New password</b></label><input value={newPassword} onChange={e => setNewPassword(e.target.value)} type="password" required minLength={12} />
        <button disabled={loading}>{loading ? 'Updating…' : 'Reset password'}</button><button type="button" className={styles.secondaryButton} onClick={() => setAuthMode('request-reset')}>Request another code</button>
      </form>}
    </section></main>;
  }
  ...
  return <main className={styles.page}>
    <header className={styles.header}>
      <div><span className={styles.eyebrow}>OWNER CONTROL PLANE</span><h1>{principal.organization_name}</h1><p>{principal.name} · {principal.role} · {commandCenter?.mode?.replaceAll('_', ' ')}</p></div>
      <div className={styles.actions}>
        <button className={styles.runButton} onClick={runAutonomousCycle} disabled={cycleRunning || loading}>{cycleRunning ? 'Running cycle…' : 'Run Autonomous Cycle'}</button>
        <button onClick={() => void loadWorkspace()} disabled={loading}>Refresh</button><button onClick={importExisting} disabled={loading}>Import existing</button><button onClick={signOut}>Sign out</button>
      </div>
    </header>
    {notice && <div className={styles.notice}>{notice}</div>}{error && <div className={styles.error}>{error}</div>}
    ...
    <section className={styles.metrics}>
      <article><span>Total leads</span><strong>{kpis?.total_leads ?? pipeline?.total_leads ?? 0}</strong></article>
      <article><span>Hot leads</span><strong>{kpis?.hot_leads || 0}</strong></article>
      <article><span>Active deals</span><strong>{kpis?.active_deals ?? pipeline?.active_deals ?? 0}</strong></article>
      <article><span>Qualified buyers</span><strong>{kpis?.qualified_buyers || 0}</strong></article>
      <article><span>Projected revenue</span><strong>{money(kpis?.projected_assignment_revenue ?? pipeline?.projected_assignment_revenue ?? 0)}</strong></article>
      <article><span>Weighted revenue</span><strong>{money(kpis?.probability_weighted_revenue || 0)}</strong></article>
      <article><span>Queued tasks</span><strong>{kpis?.queued_tasks || 0}</strong></article>
      <article><span>Pending approvals</span><strong>{kpis?.pending_approvals || 0}</strong></article>
    </section>

    <section className={styles.grid}>
      <article className={styles.card}><div className={styles.cardHeader}><div><span className={styles.eyebrow}>ACQUISITIONS</span><h2>Priority seller opportunities</h2></div><small>Ranked by urgency and lead score</small></div>
        <div className={styles.list}>{commandCenter?.priority_leads.length ? commandCenter.priority_leads.map(item => <div key={item.lead_id}><span><b>{item.seller_name}</b><small>{item.source} · {item.status} · {item.timeline_days ?? '—'} days</small></span><span className={styles.score}>P{item.priority} · {Math.round(item.score)}</span></div>) : <p>No prioritized leads yet.</p>}</div>
      </article>
      <article className={styles.card}><div className={styles.cardHeader}><div><span className={styles.eyebrow}>RISK</span><h2>Deal risk queue</h2></div><small>Highest risk first</small></div>
        <div className={styles.list}>{commandCenter?.deal_risk.length ? commandCenter.deal_risk.map(item => <div key={item.deal_id}><span><b>Deal #{item.deal_id} · {item.property.city || 'Unknown'}, {item.property.state || ''}</b><small>{item.stage} · {item.probability_to_close}% close probability · {money(item.projected_assignment_fee)}</small><small>{item.next_action}</small></span><span className={styles.risk}>Risk {Math.round(item.risk_score)}</span></div>) : <p>No active deal risks.</p>}</div>
      </article>
    </section>

    <section className={styles.cardWide}><div className={styles.cardHeader}><div><span className={styles.eyebrow}>AI WORKFORCE</span><h2>Agent health</h2></div><small>Last command-center refresh: {formatDate(commandCenter?.generated_at)}</small></div>
      <div className={styles.agentGrid}>{commandCenter?.agent_health.map(agent => <article key={agent.name} className={styles.agentCard}><div><b>{agent.name.replaceAll('-', ' ')}</b><span className={`${styles.health} ${styles[agent.health] || ''}`}>{agent.health}</span></div><p>{agent.role}</p><small>Last run: {formatDate(agent.last_run_at)}{agent.confidence != null ? ` · ${Math.round(agent.confidence * 100)}% confidence` : ''}</small></article>)}</div>
    </section>

    <section className={styles.grid}>
      <article className={styles.card}><div className={styles.cardHeader}><div><span className={styles.eyebrow}>CONTROL GATE</span><h2>Pending approvals</h2></div><strong>{commandCenter?.approvals.length || 0}</strong></div>
        <div className={styles.list}>{commandCenter?.approvals.length ? commandCenter.approvals.map(item => <div key={item.id}><span><b>{item.action_type.replaceAll('_', ' ')}</b><small>{item.summary}</small><small>{formatDate(item.created_at)}</small></span><span className={styles.decisionButtons}><button onClick={() => void decideApproval(item.id, 'approved')} disabled={loading}>Approve</button><button onClick={() => void decideApproval(item.id, 'rejected')} disabled={loading}>Reject</button></span></div>) : <p>No approvals waiting.</p>}</div>
      </article>
      <article className={styles.card}><div className={styles.cardHeader}><div><span className={styles.eyebrow}>EXECUTION</span><h2>Autonomous queue</h2></div><strong>{commandCenter?.queue.length || 0}</strong></div>
        <div className={styles.list}>{commandCenter?.queue.length ? commandCenter.queue.map(item => <div key={item.id}><span><b>{item.task_type.replaceAll('_', ' ')}</b><small>Task #{item.id} · Lead {item.lead_id || '—'} · {formatDate(item.created_at)}</small></span><span className={styles.score}>P{item.priority}</span></div>) : <p>No queued work.</p>}</div>
      </article>
    </section>
