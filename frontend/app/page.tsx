...
type View = 'Command Center' | 'Leads' | 'Cash Buyers' | 'Underwriting' | 'Buyer Matching' | 'Bland Calls' | 'Driving for Dollars';

const navItems: View[] = ['Command Center', 'Leads', 'Cash Buyers', 'Underwriting', 'Buyer Matching', 'Bland Calls', 'Driving for Dollars'];
...
export default function Home() {
  ...
  async function createLead(event: FormEvent<HTMLFormElement>, source = 'manual') {
    ...
    await post(source === 'driving_for_dollars' ? '/driving-for-dollars' : '/leads', {
      ...
      motivation_score: Number(form.get('motivation_score') || 0), equity_score: Number(form.get('equity_score') || 0),
      timeline_days: Number(form.get('timeline_days') || 30), notes: form.get('notes') || null,
      property: {
        ...
        sqft: Number(form.get('sqft') || 0), asking_price: Number(form.get('asking_price') || 0), arv: Number(form.get('arv') || 0),
        repairs: Number(form.get('repairs') || 0),
        distress_signals: String(form.get('distress_signals') || '').split(',').map(value => value.trim()).filter(Boolean),
  ...
  async function createBuyer(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(formElement);
    await post('/buyers', {
      name: form.get('name'), company: form.get('company') || null, buyer_type: form.get('buyer_type'), phone: form.get('phone'),
      email: form.get('email') || null, zip_codes: String(form.get('zip_codes') || '').split(',').map(value => value.trim()).filter(Boolean),
  ...
  return (
    <main className="shell">
      ...
      <section className="workspace">
        ...
        {active === 'Command Center' && <CommandCenter stats={stats} leads={leads} apiStatus={apiStatus} onNavigate={setActive} onRefresh={refresh} />}
