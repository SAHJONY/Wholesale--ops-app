...
export default function JointVentureForm(){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      const response=await fetch('/api/backend/joint-venture-public',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      ...
      setStatus('success');setMessage('JV opportunity received. SAHJONY Operations will review authority, economics, buyer coverage, title path, and proposed split before any agreement is created.');form.reset();
  ...
  return <form className="publicForm" onSubmit={submit}>
    ...
    <div className="formGrid">
      ...
      <label className="field"><span>Seller / contract price</span><input name="contract_price" inputMode="decimal" placeholder="$75,000"/></label>
      <label className="field"><span>Buyer / disposition price</span><input name="buyer_price" inputMode="decimal" placeholder="$105,000"/></label>
