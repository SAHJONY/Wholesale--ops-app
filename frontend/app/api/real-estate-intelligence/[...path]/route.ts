...
async function handler(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  try {
    const response = await fetch(`${BACKEND}/real-estate-intelligence/${route}`, {
    ...
    if (response.status === 404) {
      return NextResponse.json({ status: 'setup', providers: {}, workflow: {}, summary: {}, acquisition_runs: {}, opportunities: [], detail: 'Integrated Intelligence API is not deployed yet.' }, { status: 200 });
    }
    return new NextResponse(text, { status: response.status, headers: { 'Content-Type': 'application/json' } });
  } catch {
    return NextResponse.json({ status: 'setup', providers: {}, workflow: {}, summary: {}, acquisition_runs: {}, opportunities: [], detail: 'Integrated Intelligence API is unavailable.' }, { status: 200 });
  }
}
