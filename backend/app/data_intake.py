...
OPTIONAL_COLUMNS = {
    "email", "source", "notes", "asking_price", "arv", "repairs", "bedrooms",
    "bathrooms", "sqft", "motivation_score", "distress_score", "equity_score",
}
MAX_ROWS = 5000
...
def _address_key(row: dict) -> str:
    return "|".join([
        re.sub(r"\s+", " ", _text(row.get("address")).lower()),
        _text(row.get("city")).lower(),
        _text(row.get("state")).upper(),
        _text(row.get("zip_code"))[:5],
    ])
...
def _existing_keys(db: Session, organization_id: int) -> tuple[set[str], set[str]]:
    ...
    phones = {_phone(lead.phone) for lead in leads if _phone(lead.phone)}
    addresses = {_address_key({"address": p.address, "city": p.city, "state": p.state, "zip_code": p.zip_code}) for p in properties}
...
def _preview_rows(db: Session, principal: Principal, csv_text: str) -> dict:
    ...
    for row in rows:
        ...
        if not all(_text(row.get(name)) for name in ["seller_name", "address", "city", "state", "zip_code"]):
            errors.append("Required identity or property fields are missing")
        ...
        seen_phones.add(phone); seen_addresses.add(address_key)
    return {"generated_at": datetime.utcnow(), "headers": headers, "total_rows": len(rows), "accepted_count": len(accepted), "duplicate_count": len(duplicates), "rejected_count": len(rejected), "accepted": accepted[:100], "duplicates": duplicates[:100], "rejected": rejected[:100]}
...
def snapshot(principal: Principal = Depends(get_principal), db: Session = Depends(get_db)):
    imports = db.scalars(select(CrmActivity).where(
        ...
        CrmActivity.activity_type == "data_intake_import",
...
def commit(payload: dict, principal: Principal = Depends(require_role("manager")), db: Session = Depends(get_db)):
    ...
    source_label = _text(payload.get("source_label")) or "csv_import"
    preview_result = _preview_rows(db, principal, csv_text)
    accepted_rows = {item["row_number"] for item in preview_result["accepted"]}
    rows, _ = _parse(csv_text)
    created = []
    for row in rows:
        if row["row_number"] not in accepted_rows:
            continue
        lead = Lead(
            ...
            motivation_score=_number(row.get("motivation_score")) or 0,
            distress_score=_number(row.get("distress_score")) or 0,
            equity_score=_number(row.get("equity_score")) or 0,
        )
        property_row = Property(
            address=_text(row.get("address")), city=_text(row.get("city")), state=_text(row.get("state")).upper(), zip_code=_text(row.get("zip_code")),
            asking_price=_number(row.get("asking_price")), arv=_number(row.get("arv")), repairs=_number(row.get("repairs")),
            bedrooms=_number(row.get("bedrooms"), int), bathrooms=_number(row.get("bathrooms")), sqft=_number(row.get("sqft"), int),
        )
        lead.property = property_row
        db.add(lead); db.flush()
        db.add_all([
            WorkspaceEntity(organization_id=principal.organization_id, entity_type="lead", entity_id=lead.id),
            WorkspaceEntity(organization_id=principal.organization_id, entity_type="property", entity_id=property_row.id),
        ])
        created.append({"lead_id": lead.id, "property_id": property_row.id})
    audit = CrmActivity(
        organization_id=principal.organization_id,
        user_id=principal.user_id,
        activity_type="data_intake_import",
        summary=f"Imported {len(created)} production lead(s) from {source_label}",
        metadata_json={"source_label": source_label, "total_rows": preview_result["total_rows"], "created": len(created), "duplicates": preview_result["duplicate_count"], "rejected": preview_result["rejected_count"], "texas_excluded": True},
    )
