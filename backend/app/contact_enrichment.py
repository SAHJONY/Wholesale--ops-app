...
def _contact_review(lead: Lead, evidence: dict) -> dict:
    ...
    if evidence.get("owner_name") and lead.seller_name and evidence["owner_name"].strip().lower() != lead.seller_name.strip().lower():
        conflicts.append({
            "field": "owner_vs_seller",
            "current": lead.seller_name,
            "incoming": evidence["owner_name"],
            "requires": "right_party_and_county_record_verification",
        })
    ...
    return {
        ...
        "requires_human_review": bool(conflicts or compliance_blocks),
        "outreach_allowed": bool(evidence.get("outreach_allowed")) and not compliance_blocks,
    }
...
async def _lookup_for_lead(lead: Lead) -> dict:
    ...
    try:
        return await lookup_batchdata_contacts(
...
async def preview_batchdata_enrichment(
    lead_id: int,
...
async def apply_batchdata_enrichment(
    lead_id: int,
    ...
    if safe_phones:
        ...
        if not lead.phone or force or "phone" in approved_fields:
            if "phone" not in conflict_fields or force or "phone" in approved_fields:
                lead.phone = incoming_phone
                applied["phone"] = incoming_phone
    if emails:
        incoming_email = emails[0]["email"]
        if not lead.email or force or "email" in approved_fields:
            if "email" not in conflict_fields or force or "email" in approved_fields:
    ...
    activity = CrmActivity(
        ...
        summary=f"BatchData evidence reviewed for {lead.seller_name}; {len(applied)} fields applied",
        metadata_json={
            "provider": "batchdata",
            "observed_at": evidence.get("observed_at") or datetime.utcnow().isoformat(),
            "confidence": evidence.get("confidence"),
            "owner_name": evidence.get("owner_name"),
            "phones": evidence.get("phones"),
            "emails": evidence.get("emails"),
            "blocked_phone_candidates": evidence.get("blocked_phone_candidates"),
            ...
            "compliance_blocks": review.get("compliance_blocks"),
    ...
    return {
        ...
        "compliance_blocks": review["compliance_blocks"],
        "outreach_allowed": outreach_allowed,
        "note": "Applying contact data never initiates outreach; campaigns remain approval-gated.",
    }
