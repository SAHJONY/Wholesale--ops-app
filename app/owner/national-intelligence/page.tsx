...
const empty:Snapshot={summary:{properties:0,high_priority:0,ownership_verified:0,projected_assignment_fees:0,average_opportunity_score:0},properties:[],markets:[],runs:[]};
...
export default function NationalIntelligencePage(){
  ...
  const load=useCallback(async(override?:string)=>{setLoading(true);setError('');try{setData(await request('/snapshot',{},override));}catch(e){setError(e instanceof Error?e.message:'Unable to load national intelligence');}finally{setLoading(false);}},[request]);
